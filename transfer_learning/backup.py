'''
Resnet (with some changes in full connected layer) Estimator for X-ray image classification.
'''
import numpy as np
...
def resnet34(inputs):
    ...
    for i in range(3):
        ...
        if i == 0:
            x = resnet_block(x, 64, [1, 1], 1, activation=None)
        x = tf.math.add(x, b)
        x = tf.nn.relu(x)

    for i in range(4):
        if i == 0:
            a = resnet_block(x, 128, [3, 3], 2)
        else:
            a = resnet_block(x, 128, [3, 3], 1)
        b = resnet_block(a, 128, [3, 3], 1, activation=None)
        if i == 0:
            x = resnet_block(x, 128, [1, 1], 2, activation=None)
        x = tf.math.add(x, b)
        x = tf.nn.relu(x)

    for i in range(6):
        if i == 0:
            a = resnet_block(x, 256, [3, 3], 2)
        else:
            a = resnet_block(x, 256, [3, 3], 1)
        b = resnet_block(a, 256, [3, 3], 1, activation=None)
        if i == 0:
            x = resnet_block(x, 256, [1, 1], 2, activation=None)
        x = tf.math.add(x, b)
        x = tf.nn.relu(x)
    ...
    for i in range(3):
        ...
        if i == 0:
            x = resnet_block(x, 512, [1, 1], 2, activation=None)
        x = tf.math.add(x, b)
        x = tf.nn.relu(x)

    x = tf.layers.AveragePooling2D(pool_size=10, strides=1, data_format='channels_last')(x)
...
def resnet154(inputs):
    ...
    for i in range(3):
        ...
        if i == 0:
            x = resnet_block(x, 256, [1, 1], 1, activation=None)
        x = tf.math.add(x, c)
        x = tf.nn.relu(x)
    ...
    for i in range(36):
        ...
        if i == 0:
            x = resnet_block(x, 1024, [1, 1], 2, activation=None)
        x = tf.math.add(x, c)
        x = tf.nn.relu(x)
    ...
    for i in range(3):
        ...
        if i == 0:
            x = resnet_block(x, 2048, [1, 1], 2, activation=None)
        x = tf.math.add(x, c)
        x = tf.nn.relu(x)
...
def cnn_model_fn(features, labels, mode):
    ...
    label_tensor = tf.convert_to_tensor(labels['label'], dtype=tf.int64)
    ...
    # loss = loss_thigh + thigh_shin_ration * loss_shin
    # Regularization loss
    ...
    recall = tf.metrics.recall(label_tensor, predicted_classes)
    precision = tf.metrics.precision(label_tensor, predicted_classes)
    ...
    eval_metric_ops = {
            ...
            'precison': tf.metrics.mean(precision),
...
def _parse_function(record):
    ...
    features = {'index': parsed_features['index'],
            ...
            'label': parsed_features['label'], # just for prediction ouput, DO NOT use it for training
            }
    labels = {'label':parsed_features['label']}
